 ...
 * 
 * 类名：SocketServerKDUDP.java
 * 多线程：http://www.importnew.com/26678.html
 * 作者：Administrator 创建时间：Jan 6, 2012 5:16:28 PM
 * 
 * 修改人： 修改时间：
 */
public class SocketServerKDUDP {
	Logger logger = Logger.getLogger(SocketServerKDUDP.class);
	...
	public SocketServerKDUDP() throws Exception{
		try {
	...
	public void service() {
		...
		while (true) { 
			try {
				 ...
				 executorService.execute(new KDUdpCallService(dataPacket));//接收到消息后执行具体的业务
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
